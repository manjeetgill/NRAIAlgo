...
import { downloadEodAuditPack } from "./export-client";
...
export function MarketClosedScreen({ snapshot, layout, accountHoldings, accountPositions }: { snapshot: OverviewSnapshot; layout?: "after-close" | "weekend-holiday"; accountHoldings?: { rows: PortfolioRow[]; complete: boolean }; accountPositions?: { rows: OpenPositionView[]; complete: boolean; includesIcici: boolean } }) {
  ...
  const summaryPnl = pnl?.netPaise ?? pnl?.grossPaise ?? openPositionPnl;
  ...
  const reportedBrokerAccounts = providerRows.filter(row => row.connection || row.positions > 0 || row.holdings > 0).length;
  const calculation = snapshot.calculationReconciliation;
  ...
  return <div className={`${shared.terminal} ${styles.closed} ${weekend ? styles.weekend : ""}`}>
    ...
    <section className={styles.executiveDeck} aria-label="Closed market status">
      <div className={styles.deckHeader}><div className={styles.deckMain}><div className={styles.certLine} data-state={officialCloseMatched ? "matched" : "pending"}><span>Official EOD reference reconciliation</span><small>Snapshot {snapshot.snapshotId}</small><b><i />{officialCloseMatched ? "Bhavcopy matched" : "Bhavcopy verification pending"}</b></div><div className={styles.marketRefs}><div><strong>EOD indices</strong><span>{officialCloseMatched ? "Official closes" : "Verified values only"}</span></div>{marketReferenceTiles.map(({id,label,quote}) => <section key={id}><h2>{label}</h2><strong>{quote ? quote.value.toLocaleString("en-IN", {minimumFractionDigits:2,maximumFractionDigits:2}) : "—"}</strong><span>{quote ? `${quote.change == null ? "" : `${quote.change >= 0 ? "+" : ""}${quote.change.toFixed(2)}${quote.changePct == null ? "" : ` (${quote.changePct >= 0 ? "+" : ""}${quote.changePct.toFixed(2)}%)`} · `}${quote.priceBasis.replaceAll("-", " ")} · ${formatTimestamp(quote.sourceAsOf)} IST` : "Source unavailable"}</span></section>)}</div><h2>Post-Market Surveillance &amp; Scenario Deck</h2><p>NRI Institutional Pro-Desk · Multi-broker aggregate risk, overnight exposure &amp; next-session pre-flight</p></div><div className={styles.deckActions}><button type="button" onClick={() => downloadEodAuditPack(snapshot)}>Export EOD Audit Pack</button><Link href="/app/broker-connections">Re-authenticate brokers</Link></div></div>
      <div className={styles.metricDeck}>
        <section><h2 className={shared.srOnly}>{weekend ? "Accumulated Performance" : "Session P&L Summary"}</h2><header><span>{reconciled ? "Realized net P&L" : pnl ? "Broker-reported gross P&L" : "Open-position P&L subtotal"}</span><b>{reconciled ? "Reconciled" : pnl ? snapshot.pnl.status === "degraded" ? "Partial coverage" : "Provisional / pending" : openPnlComplete ? "All reported positions" : "Partial position coverage"}</b></header><div className={(summaryPnl ?? 0) < 0 ? shared.negative : shared.positive}><Money value={summaryPnl} /></div><small>Reported period: {pnl?.period ?? "Current open positions"}</small><small>{pnl ? <><Money value={pnl.grossPaise} /> gross · {pnl.chargesPaise == null ? "charges pending" : `${formatPaise(pnl.chargesPaise)} charges`}</> : `${reportedOpenPnl.length}/${activePositions.length} positions · before charges`}</small><div className={styles.metricProgress}><i style={{width:pnl ? "82%":reportedOpenPnl.length ? "42%":"0%"}} /></div><details><summary>Performance evidence</summary><div className={styles.statistics}><div><span>Day win rate</span><strong>{perf?.dayWinRatePct != null ? `${perf.dayWinRatePct}%` : "—"}</strong></div><div><span>Profit factor</span><strong>—</strong></div><div><span>Sharpe</span><strong>{perf?.sharpe != null ? perf.sharpe.toFixed(2) : perf ? `${perf.sessionsRecorded}/${perf.sessionsRequiredForSharpe} sessions` : "—"}</strong></div><div><span>Max drawdown</span><strong>{perf?.maxDrawdownPaise != null ? formatPaise(perf.maxDrawdownPaise) : "—"}</strong></div></div>{weekend && <p>Weekly equity history unavailable</p>}<Source panel={snapshot.pnl} /></details></section>
        ...
        <section><h2 className={shared.srOnly}>{weekend ? "Execution Engines" : "Strategy Posture"}</h2><header><span>Theta accrual</span><b>Analytics source required</b></header><div className={shared.muted}>Unavailable</div><small>Projected overnight decay requires a licensed Greeks source.</small><div className={styles.metricProgress}><i style={{width:"0%"}} /></div><p className={styles.exposure}>{activePositions.length ? `${activePositions.length} positions reported; unreported exposure is not inferred.` : "Overnight exposure unverified. Market close does not flatten positions."}</p><Link className={shared.button} href="/app/broker-connections">Review execution prerequisites</Link></section>
    ...
    {act === "settle" && <div className={styles.actPanel} role="tabpanel" aria-label="Settle & Reconcile">
      <div className={styles.settlementGrid}>
        ...
        <Card title="NRI Broker Settlements" badge={`${reportedBrokerAccounts} broker accounts reported`}>
        ...
        <Card title="Calculation & Reconciliation" badge={calculation?.data?.overallStatus ?? "Unavailable"}>
          <p className={styles.sectionHint}>Independent arithmetic checks; broker values remain authoritative and are never silently overwritten.</p>
          <div className={styles.complianceList}>{calculation?.data?.checks.map(check => <div key={check.id}><span>{check.label}<small>{check.reason}</small></span><strong className={check.status === "matched" ? shared.positive : check.status === "mismatch" ? shared.negative : shared.warning}>{check.status.replaceAll("_", " ")}{check.difference == null ? "" : ` · Δ ${check.unit === "paise" ? formatPaise(check.difference) : check.difference}`}</strong></div>) ?? <div><span>Reconciliation service</span><strong>Unavailable</strong></div>}</div>
          {calculation && <Source panel={calculation} />}
        </Card>
      </div>
      <Card title="Demat Long Cash Holdings & Pledged Collateral Haircut" badge={complete ? `${Math.min(3, holdingRows.length)} of ${holdingRows.length} shown` : "Partial coverage"}><PortfolioTable rows={holdingRows.slice(0, 3)} available={complete} /></Card>
    </div>}
    ...
    {act === "tomorrow" && <div className={styles.actPanel} role="tabpanel" aria-label="Tomorrow's Edge">
      <div className={styles.tomorrowGrid}>
        ...
        <Card title="Tomorrow Pre-Flight Checklist" badge={`${passed} of 4 ready`}><p className={styles.sectionHint}>Revalidate before {session?.nextSession ?? "the next session"}.</p><div className={styles.preflightList}>{checks.map(({label,check}) => <div key={label}><span><strong>{label}</strong><small>{check.reason}</small></span><b className={check.status === "passed" ? shared.positive : check.status === "failed" ? shared.negative : shared.warning}>{STATUS[check.status]}</b></div>)}</div><button className={shared.button} type="button" onClick={() => window.location.reload()}>Re-run pre-flight checks</button></Card>
      </div>
