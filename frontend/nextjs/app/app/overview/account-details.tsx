...
export function AccountDetails({ snapshot, broker, account, status, iciciStale = false }: { snapshot: OverviewSnapshot; broker: BrokerView; account: IciciAccount | null; status: string; iciciStale?: boolean }) {
  ...
  const selected = selectedProviders(snapshot, broker);
  ...
  const iciciFundsRows = include ? (account?.sections.funds?.rows ?? []) : [];
  const hasFunds = fundsRows.length > 0 || iciciFundsRows.length > 0;
  const representedFunds = new Set([...fundsRows.map(row => row.provider), ...(iciciFundsRows.length ? ["icici"] : [])]);
  const missingFunds = selected.filter(provider => !representedFunds.has(provider));
  ...
  return <>
    ...
    <section className={styles.card}><h2>Broker Funds</h2><Link href="/app/funds-margin">View all funds & margin →</Link><div className={styles.tableScroll}><table><thead><tr><th>Broker / account</th><th>Balance type</th><th>Amount</th></tr></thead><tbody>
      ...
      {iciciFundsRows.map((row, index) => <tr key={index}><td>ICICI · {account?.accountId}</td><td>Bank balance</td><td>{row.total_bank_balance == null || String(row.total_bank_balance).trim() === "" || !Number.isFinite(Number(row.total_bank_balance)) ? "Unavailable" : formatPaise(Math.round(Number(row.total_bank_balance) * 100))}</td></tr>)}
      {missingFunds.map(provider => <tr key={`missing:${provider}`}><td>{provider === "icici" ? "ICICI" : provider} · {provider === "icici" ? account?.accountId ?? "account unavailable" : "account unavailable"}</td><td>Funds coverage</td><td>Unavailable</td></tr>)}
