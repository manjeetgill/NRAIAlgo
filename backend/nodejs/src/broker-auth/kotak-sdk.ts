...
export function startKotakSdk(op:'login'|'portfolio'|'stream', data:unknown, receive:(value:unknown)=>void, failed:(code?:KotakSdkErrorCode)=>void) {
  const child=spawn(process.env.KOTAK_SDK_PYTHON ?? resolve(root,'backend/python/kotak-sdk/.venv/bin/python'),[resolve(root,'backend/python/kotak-sdk/bridge.py')],{stdio:['pipe','pipe','pipe'],env:{PATH:process.env.PATH,HOME:process.env.HOME,LANG:process.env.LANG,PYTHONUNBUFFERED:'1',PYTHONDONTWRITEBYTECODE:'1'}});
