...
vi.mock('../market-data/live-overview.js',()=>({LiveOverview:class{constructor(){return managers.zerodha;}}}));
vi.mock('../market-data/kotak-live-overview.js',()=>({KotakLiveOverview:class{constructor(){return managers.kotak;}}}));
vi.mock('../build-overview-snapshot.js',()=>({buildOverviewSnapshot:vi.fn(),loadOverviewInputs:vi.fn()}));
vi.mock('./auth.js',()=>({requireAuth:()=>async(request:{auth?:unknown})=>{request.auth={workspaceId:'review-workspace',accountId:'A',email:'test@example.invalid'};}}));
